...
/*
___________________	GF_AP Units Map Markers and Symbols Script - Mod	___________________

https://forums.bohemia.net/forums/topic/219763-GF_AP-units-map-markers-and-symbols-script/
...
For the Compilation List of my GF_AP Scripts , you can search in:
https://forums.bohemia.net/forums/topic/215850-compilation-list-of-my-GF_AP-scripts/
*/


//___________________	You can add in the init of a unit to exclude	___________________	
//	this setVariable ["Var_GF_AP_Units_Map_Markers", true];
//	this setVariable ["Var_GF_AP_Units_Map_Symbols", true];
...
if (GF_AP_Units_Map_systemchat) then {
systemchat "GF_AP Units Map Markers and Symbols Script Initializing";
};


if (GF_AP_Units_Map_diag_log) then {
diag_log "//___________________	GF_AP_Units_Map_Markers_and_Symbols.sqf	_____________";
diag_log "//___________________ GF_AP Units Map Markers and Symbols Script Initializing _____________";
};




GF_AP_Units_Map_Markers_Number = 0; 

GF_AP_Units_Map_Markers = { 


_this addEventHandler ["Respawn", {
	params ["_unit", "_corpse"];
	_unit setVariable ["Var_GF_AP_Units_Map_Markers",false];
}];

private ["_marker" , "_name" , "_Leader"];
	
GF_AP_Units_Map_Markers_Number = GF_AP_Units_Map_Markers_Number + 1;
_marker = format["GF_AP_Units_Map_Markers_Number%1",GF_AP_Units_Map_Markers_Number];
...
if (!isPlayer _this) then {		
	...
	if (		
		((alive _this)) 
		&& ((_this getVariable ["Var_GF_AP_Units_Map_Markers",true]))		
		) then {
			...
			if (GF_AP_Units_Map_Symbols_Remove_Text)then { 
			//	Remove_Text
			}else{
...
GF_AP_Units_Map_Symbols = { 

_this addEventHandler ["Respawn", {
	params ["_unit", "_corpse"];
	_unit setVariable ["Var_GF_AP_Units_Map_Symbols",false];
}];
...
setGroupIconsVisible [ true, true ];
	...
	if (		
		((alive _this)) 
		&& ((_this getVariable ["Var_GF_AP_Units_Map_Symbols",true]))		
		) then {
			...
			_this setVariable ["Var_GF_AP_Units_Map_Symbols_Leader",true];
			...
			((_Leader)) && (count (units group _this) isEqualto 1)
			&& (!(vehicle _this isKindOf  "Man")) 
			&& (!(_this getVariable ["Var_GF_AP_Units_Map_Symbols_Leader",true]))
			...
			_this setVariable ["Var_GF_AP_Units_Map_Symbols_Leader",false];
			clearGroupIcons (group _this);		
			_true = true;
			uisleep _sleep; 
			};
...
[] spawn {
	...
	{	
		if (
		((alive _x))
		&& (!(_x getVariable ["Var_GF_AP_Units_Map_Markers",false]))
		&& (!(_x getVariable ["Var_GF_AP_Units_Map_Symbols",false]))		
		...
		*/
		...
		if (GF_AP_Units_Map_Markers_Enabled)then { 
		_x spawn GF_AP_Units_Map_Markers;
		};
		if (GF_AP_Units_Map_Symbols_Enabled)then { 
		_x spawn GF_AP_Units_Map_Symbols;
		};
		};						
		_x setVariable ["Var_GF_AP_Units_Map_Markers",true];
		_x setVariable ["Var_GF_AP_Units_Map_Symbols",true];

		{waitUntil {!alive _x};
		_x setVariable ["Var_GF_AP_Units_Map_Markers",false];
		_x setVariable ["Var_GF_AP_Units_Map_Symbols",false];
		};
	}forEach allUnits;
	};
};
...
if (GF_AP_Units_Map_systemchat) then {
systemchat "GF_AP Units Map Markers and Symbols Script Initialized";
};


if (GF_AP_Units_Map_diag_log) then {
diag_log "//___________________ GF_AP Units Map Markers and Symbols Script Initialized _____________";
};