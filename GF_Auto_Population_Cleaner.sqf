...
[]spawn	{
	while {true} do {
		...
		while {true} do {uisleep GF_AP_Cleaner_Loop;
		
		
		//____________________________	_Vehicle_list	_________________________
		
		_Vehicle_list = nearestObjects [GF_AP_centerPosition, ["Car","Tank","Air","Ship"], worldSize];	
		...
		if (
			...
			&& (_x getVariable ["Var_GF_AP_Spawn",false])
			&& ({_x distance _Vehicle < GF_AP_Cleaner_Distance}count playableUnits isEqualTo 0)
		...
		_Unit_list = GF_AP_centerPosition nearEntities [["Man"], worldSize];	
		...
		if (
			...
			&& (_x getVariable ["Var_GF_AP_Spawn",false])
			&& ({_x distance _Entity < GF_AP_Cleaner_Distance}count playableUnits isEqualTo 0)
		...
		if (
			...
			&& (!((typeOf _x) in _Vehicle_Exclude_List))  
			&& (_x getVariable ["Var_GF_AP_Spawn",false])
			&& ((_x distance player) > GF_AP_Cleaner_Distance)
			&& ((count (crew _Vehicle)) > 0)
