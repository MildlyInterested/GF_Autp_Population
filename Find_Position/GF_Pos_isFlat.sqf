...
private ["_randomPos","_Pos","_isFlat_randomPosition"];
...
if (
	...
	systemchat format ["Position isFlat Found %1",GF_AP_Safe_distance_Check_Ok];
	};	

	if (GF_AP_Diag_Log_info) then {
	diag_log format ["Position isFlat Found %1",GF_AP_Safe_distance_Check_Ok];
	};		
	};
	}else{
...
if (GF_AP_Systemchat_info) then {
systemchat "There is NO Position isFlat ! , selecting on Random"; 
};	

if (GF_AP_Diag_Log_info) then {
diag_log "There is NO Position isFlat ! , selecting on Random"; 
};
