...
private ["_randomPos","_Pos","_Hill_Position"];
...
_Hill_Position = nearestLocation [_randomPos,"Hill"];
_Pos = locationPosition _Hill_Position;
...
if !(_Pos isEqualTo [0,0,0]) then {
	...
	systemchat format ["Position Hill Found %1",GF_AP_Safe_distance_Check_Ok];
	};	

	if (GF_AP_Diag_Log_info) then {
	diag_log format ["Position Hill Found %1",GF_AP_Safe_distance_Check_Ok];
	};		
...
if (GF_AP_Systemchat_info) then {
systemchat "There is NO Position Hill ! , selecting on Random"; 
};	

if (GF_AP_Diag_Log_info) then {
diag_log "There is NO Position Hill ! , selecting on Random"; 
};
