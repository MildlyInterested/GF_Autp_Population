...
private ["_randomPos","_Pos","_NameCity_Position"];
...
_NameCity_Position = nearestLocation [_randomPos,"NameCity"];
_Pos = locationPosition _NameCity_Position;
...
if !(_Pos isEqualTo [0,0,0]) then {
	...
	systemchat format ["Position NameCity Found %1",GF_AP_Safe_distance_Check_Ok];
	};	

	if (GF_AP_Diag_Log_info) then {
	diag_log format ["Position NameCity Found %1",GF_AP_Safe_distance_Check_Ok];
	};		
...
if (GF_AP_Systemchat_info) then {
systemchat "There is NO Position NameCity ! , selecting on Random"; 
};	

if (GF_AP_Diag_Log_info) then {
diag_log "There is NO Position NameCity ! , selecting on Random"; 
};
