...
GF_AP_randomPos_AnyNearestLocation = {

 	
private ["_randomPos","_Pos","_AnyNearestLocation_Position"];
...
_AnyNearestLocation_Position = nearestLocation [_randomPos,""];
_Pos = locationPosition _AnyNearestLocation_Position;
...
if !(_Pos isEqualTo [0,0,0]) then {
	...
	systemchat format ["Position AnyNearestLocation Found %1",GF_AP_Safe_distance_Check_Ok];
	};	

	if (GF_AP_Diag_Log_info) then {
	diag_log format ["Position AnyNearestLocation Found %1",GF_AP_Safe_distance_Check_Ok];
	};		
...
if (GF_AP_Systemchat_info) then {
systemchat "There is NO Position AnyNearestLocation ! , selecting on Random"; 
};	

if (GF_AP_Diag_Log_info) then {
diag_log "There is NO Position AnyNearestLocation ! , selecting on Random"; 
};
