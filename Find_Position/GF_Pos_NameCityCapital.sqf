...
private ["_randomPos","_Pos","_NameCityCapital_Position"];
...
_NameCityCapital_Position = nearestLocation [_randomPos,"NameCityCapital"];
_Pos = locationPosition _NameCityCapital_Position;
...
if !(_Pos isEqualTo [0,0,0]) then {
	...
	systemchat format ["Position NameCityCapital Found %1",GF_AP_Safe_distance_Check_Ok];
	};	

	if (GF_AP_Diag_Log_info) then {
	diag_log format ["Position NameCityCapital Found %1",GF_AP_Safe_distance_Check_Ok];
	};		
...
if (GF_AP_Systemchat_info) then {
systemchat "There is NO Position NameCityCapital ! , selecting on Random"; 
};	

if (GF_AP_Diag_Log_info) then {
diag_log "There is NO Position NameCityCapital ! , selecting on Random"; 
};
