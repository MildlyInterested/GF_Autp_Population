...
private ["_randomPos","_Pos","_Airport_Position"];
...
_Airport_Position = nearestLocation [_randomPos,"Airport"];
_Pos = locationPosition _Airport_Position;
...
if !(_Pos isEqualTo [0,0,0]) then {
	...
	systemchat format ["Position Airport Found %1",GF_AP_Safe_distance_Check_Ok];
	};	

	if (GF_AP_Diag_Log_info) then {
	diag_log format ["Position Airport Found %1",GF_AP_Safe_distance_Check_Ok];
	};		
...
if (GF_AP_Systemchat_info) then {
systemchat "There is NO Position Airport ! , selecting on Random"; 
};	

if (GF_AP_Diag_Log_info) then {
diag_log "There is NO Position Airport ! , selecting on Random"; 
};
