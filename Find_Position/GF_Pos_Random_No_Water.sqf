...
if !(_Pos isEqualTo [0,0,0]) then {
	...
	systemchat format ["Position Random_No_Water Found %1",GF_AP_Safe_distance_Check_Ok];
	};	

	if (GF_AP_Diag_Log_info) then {
	diag_log format ["Position Random_No_Water Found %1",GF_AP_Safe_distance_Check_Ok];
	};		
...
if (GF_AP_Systemchat_info) then {
systemchat "There is NO Position Random_No_Water ! , selecting on Random"; 
};	

if (GF_AP_Diag_Log_info) then {
diag_log "There is NO Position Random_No_Water ! , selecting on Random"; 
};
