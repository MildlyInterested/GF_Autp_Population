...
private ["_randomPos","_Pos","_NameMarine_Position"];
...
_NameMarine_Position = nearestLocation [_randomPos,"NameMarine"];
_Pos = locationPosition _NameMarine_Position;
...
if !(_Pos isEqualTo [0,0,0]) then {
	...
	systemchat format ["Position NameMarine Found %1",GF_AP_Safe_distance_Check_Ok];
	};	

	if (GF_AP_Diag_Log_info) then {
	diag_log format ["Position NameMarine Found %1",GF_AP_Safe_distance_Check_Ok];
	};		
...
if (GF_AP_Systemchat_info) then {
systemchat "There is NO Position NameMarine ! , selecting on Random"; 
};	

if (GF_AP_Diag_Log_info) then {
diag_log "There is NO Position NameMarine ! , selecting on Random"; 
};
