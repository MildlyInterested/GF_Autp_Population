...
private ["_randomPos","_Pos","_NameLocal_Position"];
...
_NameLocal_Position = nearestLocation [_randomPos,"NameLocal"];
_Pos = locationPosition _NameLocal_Position;
...
if !(_Pos isEqualTo [0,0,0]) then {
	...
	systemchat format ["Position NameLocal Found %1",GF_AP_Safe_distance_Check_Ok];
	};	

	if (GF_AP_Diag_Log_info) then {
	diag_log format ["Position NameLocal Found %1",GF_AP_Safe_distance_Check_Ok];
	};		
...
if (GF_AP_Systemchat_info) then {
systemchat "There is NO Position NameLocal ! , selecting on Random"; 
};	

if (GF_AP_Diag_Log_info) then {
diag_log "There is NO Position NameLocal ! , selecting on Random"; 
};
