...
private ["_randomPos","_Pos","_isFlatEmpty_randomPosition"];
...
if (
	...
	systemchat format ["Position isFlatEmpty Found %1",GF_AP_Safe_distance_Check_Ok];
	};	

	if (GF_AP_Diag_Log_info) then {
	diag_log format ["Position isFlatEmpty Found %1",GF_AP_Safe_distance_Check_Ok];
	};		
	};
	}else{
...
if (GF_AP_Systemchat_info) then {
systemchat "There is NO Position isFlatEmpty ! , selecting on Random"; 
};	

if (GF_AP_Diag_Log_info) then {
diag_log "There is NO Position isFlatEmpty ! , selecting on Random"; 
};
